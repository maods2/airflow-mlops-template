...
-- Environment parameters: test-project-456, raw_data_test, warehouse_test, {{ ds }}

SELECT 
  *,
  CURRENT_TIMESTAMP() as ingestion_timestamp
FROM `test-project-456.raw_data_test.raw_data`
WHERE DATE(created_at) = '{{ ds }}'
",
        destination_dataset_table="test-project-456.warehouse_test.raw_data_{{ ds_nodash }}",
...
-- Environment parameters: test-project-456, warehouse_test, {{ ds }}
...
FROM `test-project-456.warehouse_test.raw_data_{{ ds_nodash }}`
",
        destination_dataset_table="test-project-456.warehouse_test.validated_data_{{ ds_nodash }}",
        write_disposition="WRITE_TRUNCATE",
        use_legacy_sql=False,
        gcp_conn_id="google_cloud_default"
    )
...
INSERT INTO `test-project-456.warehouse_test.warehouse_data`
SELECT 
  id,
  email,
  name,
  ...
  ingestion_timestamp,
  data_quality_status
FROM `test-project-456.warehouse_test.validated_data_{{ ds_nodash }}`
